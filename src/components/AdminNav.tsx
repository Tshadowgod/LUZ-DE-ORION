...
const tabs = [
  ...
  { href: '/admin/anuncios',   icon: 'notifications_active', label: 'Aviso'      },
];
...
export default function AdminNav({ slot = 'top' }: { slot?: 'top' | 'bottom' }) {
  ...
  return (
      <nav className="flex-none w-full h-20 flex justify-around items-center gap-0.5 px-1.5 liquid-glass z-40 rounded-t-[2.5rem] border-t border-white/30 overflow-x-auto scroll-hide"
        style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {tabs.map(tab => {
          const active = tab.href === '/admin' ? pathname === '/admin' : pathname.startsWith(tab.href);
          return (
            <Link key={tab.href} href={tab.href} aria-current={active ? 'page' : undefined}
              className={`flex-shrink-0 flex flex-col items-center justify-center px-1.5 py-2 rounded-2xl transition-colors duration-300 ${
        ...
        <button onClick={handleLogout}
          className="flex-shrink-0 flex flex-col items-center justify-center px-1.5 py-2 rounded-2xl text-on-surface-variant/60 hover:text-red-400 transition-colors duration-300">
          <span className="material-symbols-outlined text-2xl" style={{ fontVariationSettings: "'wght' 200, 'opsz' 24" }}>
            logout
          </span>
