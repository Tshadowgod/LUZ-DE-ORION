...
export default function CartSidebar() {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/20 backdrop-blur-[2px] z-50" onClick={() => setIsOpen(false)} />
      <div className="fixed right-0 top-0 h-full w-full max-w-sm z-50 flex flex-col"
        style={{ background: 'rgba(251,249,245,0.97)', backdropFilter: 'blur(24px)', borderLeft: '0.5px solid rgba(255,255,255,0.3)' }}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-outline-variant/20">
          ...
          <button onClick={() => setIsOpen(false)}
            className="p-2 rounded-full liquid-glass-dark text-on-surface-variant hover:text-primary transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3 scroll-hide">
          {items.length === 0 ? (
            ...
            items.map(item => (
              <div key={item.id} className="liquid-glass rounded-[1.5rem] p-3 flex gap-3 items-center">
